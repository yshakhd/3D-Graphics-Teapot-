...
void TM::SetTexturedRectangle(float rectw, float recth, float tx, float ty) {
	...
	itcs[2] = V3(tx, 0.0f, 0.0f);
	itcs[3] = V3(tx, ty, 0.0f);
	itcs[0] = V3(0.0f, 0.0f, 0.0f);
	itcs[1] = V3(0.0f, ty, 0.0f);
}

void TM::RenderHW(bool iswired) {
	...
	if(iswired)
		for (int i = 0; i < trisN; i++) {
			if (texture) {
				...
				glTexCoordPointer(2, GL_FLOAT, 3 * sizeof(GL_FLOAT), itcs);
				glDisableClientState(GL_COLOR_ARRAY);
			}
			glDrawElements(GL_LINE_LOOP, 3, GL_UNSIGNED_INT, &tris[3 * i]);
		}
	else {
		if (texture) {
			// provide array of texture coordinates
