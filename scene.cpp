...
Scene::Scene() {
	...
	ppc3->C = V3(120.0f, 200.0f, 200.0f);
	...
	float rw = 200.0f;
	float rh = 200.0f;
	tms[1].SetTexturedRectangle(rw, rh, 2.5f, 2.5f);
	tms[1].SetCenter(V3(0.0f, 0.0f, -150.0f));
	tms[1].Rotate(tms[0].GetCenter(), V3(1.0f, 0.0f, 0.0f), 90.0f);
	tms[1].Translate(V3(0.0f, -30.f, 0.0f));

	tms[2].LoadBin("geometry/tree1.bin");
	tms[2].SetCenter(V3(46.0f, 40.0f, -150.0f));
	tms[2].Scale(5.f);
	tms[2].SM = 1;
	...
	textures = new FrameBuffer(100, 100, 512, 512);
	texturesN = 1;

	//textures->SetChecker(0xFF000000, 0xFFDDDDDD, 64);
	textures->LoadTiff("walldesign.tiff");
	//ppc->PositionAndOrient(V3(0.0f, 0.0f, 0.0f), V3(0.0f, 0.0f, -100.0f), V3(0.0f, 1.0f, 0.0f));
	
	tms[1].texture = &textures[0];
	tms[1].SM = 3;

	needInitHW = 1;

	lightppc = new PPC(hfov * 2.f, w, h);
	V3 lightpos = V3(0.f, 100.0f, -100.f);
	lightppc->PositionAndOrient(lightpos, tms[0].GetCenter(), V3(0.f, 1.0f, 0.f));
...
void Scene::RenderHW() {
	...
	if (needInitHW) {
		...
		GLuint texture_id[1];
		glGenTextures(texturesN, texture_id);
		...
		for (int i = 0; i < texturesN; i++) {
			...
			// pass textures[ti].pix pixels to HW to know the texels of each texture
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, textures[i].w, textures[i].h, 0, GL_RGBA, GL_UNSIGNED_BYTE, textures[i].pix);
		}
		needInitHW = 0;
	}
...
void Scene::DBG() {
	{
		ppc->PositionAndOrient(V3(0.f, 20.f, -50.f), tms[0].GetCenter(), V3(0.0f, 1.0f, 0.0f));
		hwfb->show();
		while (1) {
			Render();
			hwfb->redraw();
...

	{
		...
		float rw = 200.0f;
		float rh = 200.0f;
		tms[0].SetTexturedRectangle(rw, rh, 1.0f, 1.0f);
...
void Scene::NewButton() {
	...
	for (int i = 0; i < frames; i++) {
		*ppc = ppc1.Interpolate(ppc2, (float)i / (float)frames);
		...
		Fl::check();
		counter++;
		fps = counter * 1000 / chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - start).count();
		snprintf(title, 40, "first person HW\tFPS: %.2lf", fps);
		hwfb->label(title);
	}
	ppc->SavePPC("cam_path_a8.txt");
	for (int i = 0; i < frames; i++) {
